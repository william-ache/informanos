...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [propuestas] = await pool.query<PropuestaNecesidadRow[]>(
      `SELECT id, centro_id, necesidad_id, accion, elemento, cantidad_solicitada,
              urgencia, estado, expira_en, creado_en, 0 AS votos_si, 0 AS votos_no, 0 AS votantes
       FROM propuestas_necesidad
    ...
    const [actualizadas] = await pool.query<PropuestaNecesidadRow[]>(
    ...
    return NextResponse.json({ propuesta: mapPropuestaNecesidad(final) });
  } catch (error) {
